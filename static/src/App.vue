  ...
  <v-app>
    <div class="grid-container">
      <header class="header" role="banner">
        <div class="header__search" style="font-weight: 500">
          Deployment Manager
        </div>
        ...
        <div class="managment">
          ...
          <div class="setting">
            ...
            <v-text-field
              ...
              outlined
              name="replica_count"
              v-model="input.replica_count"
              placeholder="Replicas (Default 4)"
            ...
            <v-text-field
              ...
              outlined
              name="client_count"
              v-model="input.client_count"
              placeholder="Clients (Default 1)"
              style="max-width: 250px"
            />
            ...
            <v-btn
              ...
              v-if="alive.length === 0"
              style="font-weight: bold; font-size: 12px; background: #0cad9e; color: #fff;"
              ><v-icon dark>mdi-plus</v-icon> Create Deployment</v-btn
            >
            <v-btn
              class="ma-2"
              color="error"
              v-if="alive.length > 0"
              v-on:click="stopCall()"
              style="font-weight: bold; font-size: 12px; color: #fff;"
              ><v-icon dark>mdi-close</v-icon> Stop</v-btn
            >
            <v-btn
             icon color="green" @click="heartbeatCall()"
...
export default {
  ...
  methods: {
    createDeployment: async function () {
      var deployApi = `http://${window.location.host.split(":")[0]}:5000/deploy`;
    ...
    heartbeatAPI: async function () {
      var aliveApi =`http://${window.location.host.split(":")[0]}:5000/alive`;
    ...
    stopCall: async function () {
      var stopApi = `http://${window.location.host.split(":")[0]}:5000/stop`;
      var axios = require("axios"); // for handling weather api promise
